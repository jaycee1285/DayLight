<script lang="ts">
	...
	// Filter tasks by tag
	const taggedTasks = $derived(filterByTag(markdownStore.viewTasks, tagName));

	// Separate into completed and incomplete
	const incompleteTasks = $derived(
		filterIncomplete(taggedTasks).sort((a, b) => {
	...
	const completedTasks = $derived(
		filterCompleted(taggedTasks).sort((a, b) => {
	...
	const dedupedTasks = $derived(deduplicateByFilename(taggedTasks));
