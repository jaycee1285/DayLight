{
  description = "Wayland-Ready Tasks + Calendar + Manual Time Logging App — migrated";
  ...
  outputs = { self, nixpkgs, config, flake-utils, rust-overlay }:
    flake-utils.lib.eachDefaultSystem (system:
      let
        ...
        # DayLight = Tauri WebKit + eframe overlays; shared module covers both.
        runtimeLibs = libs.tauri ++ libs.egui;
        ...
        bunDeps = pkgs.stdenvNoCC.mkDerivation {
          ...
          outputHash = "sha256-a/2dee6cqIW8DkAWuL4SN097oAo3aiG4MTMGGG3QmBw=";
        };
      in
      {
        libs.declared = {
          categories = [ "tauri" "egui" ];
          local = [ "librsvg" "atk" "harfbuzz" ];
        };
        ...
        packages.default = pkgs.rustPlatform.buildRustPackage {
          ...
          version = "0.1.0";
          src = ./.;
          cargoHash = "sha256-7cH1KESU+ibSKZcrYmgKQCPgK2VTSTbjv+V1Qw7hIGw=";

          cargoRoot = "src-tauri";
          buildAndTestSubdir = "src-tauri";

          nativeBuildInputs = with pkgs; [
            pkg-config gobject-introspection wrapGAppsHook3 bun nodejs_22
          ];
        ...
        devShells.default = pkgs.mkShell {
          ...
          packages = with pkgs; [
            nodejs_22 bun rustToolchain cargo-watch cargo-tauri jdk17 just patchelf
          ];
          ...
          shellHook = ''
            ...
            if [ -d "${androidSdkPath}" ]; then
              ...
              ANDROID_STATUS="not found"
            fi

            echo "══════════════════════════════════════════════"
            echo "  DayLight dev environment (migrated to config-canary shared libs)"
            echo "══════════════════════════════════════════════"
