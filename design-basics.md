...
|---------------------|----------------------|----------------------|----------------------|
| Web projects        | Base16 YAML themes   | Follow the DayLight  | Default stack is     |
|                     |                      | `yamltoskeleton.ts`  | usually Astro or     |
|                     |                      | pipeline and map     | Svelte + Skeleton    |
|                     |                      | into Skeleton-style  |                      |
|                     |                      | tokens               |                      |
| Mobile apps         | Same Base16-derived  | Reuse the same       | Even in one-off      |
|                     | scheme as web        | generated CSS/theme  | apps, color          |
|                     |                      | structure used by    | variables should     |
|                     |                      | DayLight             | follow Skeleton      |
|                     |                      |                      | token naming         |
| Desktop GUI apps    | GTK color variables  | Derive from          | Consistent GTK       |
|                     |                      | GTK-style variables  | variable naming      |
|                     |                      | even when the app    | makes state and      |
|                     |                      | does not use GTK     | surface bugs easier  |
|                     |                      | directly             | to test              |
| Syntax highlighting | `.tmTheme` files     | Use for syntax       | Syntax colors should |
|                     | generated from the   | across Rust apps via | stay aligned with    |
|                     | same YAML ecosystem  | Syntect and web apps | the active theme     |
|                     |                      | via Shiki            | family               |
| TUI apps            | Kitty theme          | Treat background,    | Kitty themes are a   |
|                     | semantics            | foreground, cursor,  | useful operational   |
|                     |                      | selection, and the   | reference even when  |
|                     |                      | 16 core colors as    | implementation       |
...
| Tool or source       | Role                 |
|----------------------|----------------------|
| `~/.local/share/themes` | Main library of      |
|                      | Base16 YAML, `.yml`, |
|                      | and related theme    |
|                      | files worth          |
|                      | considering          |
| DayLight             | Current model for    |
| `yamltoskeleton.ts`  | web theme conversion |
| `~/repos/base16changer` | Useful reference for |
|                      | mapping Base16       |
|                      | themes into GTK and  |
|                      | terminal-friendly    |
|                      | variables            |
| `~/.config/gtk-4.0/gtk.css` | Local GTK variable   |
...
|---------------------|----------------|----------------------|
| Spline Sans Mono    | Spline Sans    | Current default      |
|                     |                | desktop pairing      |
| Iosevka Charon Mono | Iosevka Charon | More technical and   |
|                     |                | sharper              |
| B612 Mono           | B612           | Highest readability; |
|                     |                | cockpit-derived      |
| Fragment Mono       | Work Sans      | Most humanistic      |
|                     |                | option in this set   |
...
|-----------|------------|----------------------|
| `space-1` | 1x         | Tight related        |
|           |            | elements             |
| `space-2` | 2x         | Default intra-group  |
|           |            | spacing              |
| `space-3` | 3x         | Comfortable          |
|           |            | component spacing    |
| `space-5` | 5x         | Section and panel    |
|           |            | separation           |
| `space-8` | 8x         | Major route, hero,   |
...
|----------------------|------------|----------------------|
| Mobile               | 12px       | Derived from `4 x    |
|                      |            | 3`; works with the   |
|                      |            | preferred mobile     |
|                      |            | text baseline        |
| Desktop and TUI      | 8px        | Still snaps cleanly  |
|                      |            | to 4 while keeping   |
|                      |            | desktop layouts      |
|                      |            | practical            |
| Large-format or      | Contextual | May scale upward     |
| marketing surfaces   |            | when the smallest    |
|                      |            | text or interaction  |
|                      |            | target is            |
...
|----------------------|-----------------|
| Label and related    | `space-1`       |
| control              |                 |
| Elements within the  | `space-2`       |
| same card or form    |                 |
| group                |                 |
| Internal component   | `space-3`       |
| padding or stacked   |                 |
| control groups       |                 |
| Separation between   | `space-5`       |
| panels, cards, or    |                 |
| major sections       |                 |
| Major page bands,    | `space-8`       |
| hero spacing,        |                 |
| route-level          |                 |
| separation           |                 |
...
| Avoid                | Why                  |
|----------------------|----------------------|
| Ignoring safe areas  | Creates immediate    |
|                      | visual and           |
|                      | interaction bugs     |
| Top-heavy action     | Fights thumb         |
| placement            | ergonomics           |
| Multiple competing   | Increases cognitive  |
| primary actions      | load and hesitation  |
| Casual side-sheet    | Usually wastes space |
| usage                | and weakens visual   |
|                      | discipline           |
| Weak state           | Makes touch          |
| indication           | interfaces feel      |
|                      | ambiguous and        |
|                      | error-prone          |
...
|----------------------|----------------------|
| Astro or Svelte web  | Use Skeleton-style   |
| app                  | tokens generated     |
|                      | from Base16 YAML     |
|                      | themes               |
| Svelte + Tauri       | Reuse the DayLight   |
| mobile or desktop    | theme generation     |
|                      | approach and         |
|                      | Skeleton token       |
|                      | naming               |
| GTK or native        | Normalize around GTK |
| desktop GUI          | variable semantics   |
|                      | even outside GTK     |
| Rust or web code     | Use theme-aligned    |
| editor surfaces      | `.tmTheme` syntax    |
|                      | colors through       |
|                      | Syntect or Shiki     |
| TUI                  | Start from Kitty     |
|                      | theme semantics and  |
|                      | preserve strong      |
|                      | state contrast       |
